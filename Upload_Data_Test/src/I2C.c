...
uint8_t Send_array_i2c(uint8_t arreglo[], uint8_t num_caracteres, int8_t direccion){
    ...
    i2c_cmd_handle_t cmd = i2c_cmd_link_create();//Crea una estructura para almacenar los siguientes valores
    i2c_master_start(cmd);//Inicia la comunicacion (Bit de start)
    i2c_master_write_byte(cmd, (direccion << 1) | WRITE_BIT, ACK_CHECK_EN); //Primero envia la direccion del esclavo 
                                                                            //al cual se desea comunicar y activa el chequeo de ack
    
    for(uint8_t i=0; i<=num_caracteres; i++) //Recorre el arreglo para enviar de a un dato a la vez
	{
        i2c_master_write(cmd, &arreglo[i], sizeof(arreglo[i]), ACK_CHECK_EN); //Se envian todos los datos del arreglo y activa el 
                                                                            //chequeo de ack en cada envio
	}
    i2c_master_stop(cmd); //Detiene la comunicacion (Bit de stop)
    ret = i2c_master_cmd_begin(I2C_MASTER_NUM, cmd, 1000 / portTICK_RATE_MS);//Envia toda la estructura con todo lo anterior por 
                                                                             //los pines establecido de i2c del micro. Establece un 
                                                                             //tiempo limite para no clavar el micro si no se recibe nada
    i2c_cmd_link_delete(cmd); //Elimina la estructura
...
void Read_i2c(uint8_t *variable, int8_t direccion){
    ...
    i2c_master_start(cmd); //Inicia la comunicacion (Bit de start)
    i2c_master_write_byte(cmd, (direccion << 1) | READ_BIT, ACK_CHECK_EN); //Primero envia la direccion del esclavo al cual 
                                                                           //se desea comunicar y activa el chequeo de ack
    i2c_master_read_byte(cmd, variable, NACK_VAL); //Lee el puerto i2c para guardarlo en la varible del puntero
    i2c_master_stop(cmd); //Detiene la comunicacion (Bit de stop)
    i2c_master_cmd_begin(I2C_MASTER_NUM, cmd, 1000 / portTICK_RATE_MS); //Envia toda la estructura con todo lo anterior por
                                                                        //los pines establecido de i2c del micro
    i2c_cmd_link_delete(cmd); //Elimina la estructura
}
...
========================================================================*/
esp_err_t i2c_master_init(uint8_t I2C_MASTER_SDA_IO, uint8_t I2C_MASTER_SCL_IO)
{
